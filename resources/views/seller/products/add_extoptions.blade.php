  ...
  <div class="col-md-12">
    ...
    <div class="box box-primary">
      ...
      <div class="box-footer">
        <a href="{{route('seller.productminmax',$product->id)}}" type="button" name="button" class="btn btn-primary">{{__('admindash.actions.finish')}}</a>
        <a href="{{route('seller.add.options',$product->id)}}" type="button" name="button" class="btn btn-default">{{__('admindash.actions.back')}}</a>
      </div>
    </div>
    <!-- /.box -->
...
////////////////////////////////////////////Add New ExtOption/////////////////////////////////////////////
  ...
  $('body').on('click','.save',function(){
    ...
    if(priceError==false && quantityError==false){
      $.ajax({
        url:`{{route('seller.store.extoption')}}`,
        method:'post',
        data:{price:price,quantity:quantity,product_id:`{{$product->id}}`,ids:ids,_token:`{{csrf_token()}}`},
...
///////////////////////////////////////////////// Edit //////////////////////////////////////////////////////
  ...
  $('body').on('click','.doedit',function(){
    ...
    if(priceError==false && quantityError==false){
      $.ajax({
        url:`{{route('seller.update.extoption')}}`,
        method:'put',
        data:{price:price,quantity:quantity,id:extoptionId,_token:`{{csrf_token()}}`},
        success:function(data){
          $(self).parent().parent().find('.price').attr('disabled','disabled');
          $(self).parent().parent().find('.quantity').attr('disabled','disabled');
          $(self).addClass('hide');
