  ...
  <div class="col-md-12">
    ...
    <div class="box box-primary">
      ...
      <div class="box-footer">
        <a href="{{route('seller.add.extoption',$product->id)}}" type="button" name="button" class="btn btn-primary">{{__('admindash.actions.next')}}</a>
        <a href="{{route('seller.add.variation',$product->id)}}" type="button" name="button" class="btn btn-default">{{__('admindash.actions.back')}}</a>
      </div>
    </div>
    <!-- /.box -->
...
$('body').on('click','.addoption',function(){
  ...
  if(nameError==false && nameArError==false){
    $.ajax({
      url:`{{route('seller.var.addoptions')}}`,
      method:'post',
...
$('body').on('click','.deleteoption',function(){
    ...
    var self=this;
    $.ajax({
      url:`{{route('seller.var.removeoptions')}}`,
      method:'delete',
...
$('body').on('click','.doeditoption',function(){
  ...
  if(nameError==false && nameArError==false){
    $.ajax({
      url:`{{route('seller.var.updateoptions')}}`,
      method:'put',
      data:{id:id,name:name,name_ar:name_ar,_token:`{{csrf_token()}}`},
      success:function(data){
