                    ...
                    @foreach(maincats() as $maincat)
                      @foreach($maincat->subcats as $subcat)
                        <li class="nomargin hiddable col-lg-12">
                            <input type="radio" name="subcat" class="subcat" value="{{$subcat->id}}">
      ...
      <div id="center_column" class="center_column col-xs-12 col-sm-9 col-md-9 col-lg-9">
        <div class="breadcrumbdiv">
          <div class="breadcrumb container clearfix">
            <div class="breadcrumbs"> <a class="home" href="{{route('index')}}" title="Return to Home"><i class="icon-home"></i></a> <span class="navigation-pipe">&gt;</span>نتائج البحث</div>
          </div>
        </div>
...
$(document).ready(function(){
  ...
  $('.subcat').on('change',function(){
    ...
    $.ajax({
      url:`{!!request()->fullUrl()!!}&sort=${val}&subcat=${subcat}`,
      method:'get',
