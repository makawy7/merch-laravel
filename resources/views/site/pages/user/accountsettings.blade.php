...
<div class="col-xs-12 col-sm-9 col-md-9 col-lg-9">
  <form action="{{route('updateaccount')}}" method="post" id="updateaccountform">
    @csrf
            ...
            <div class="form-group form-error">
              ...
              <select name="code" >
                @foreach(countries() as $country)
                  <option {{old('code')==$country->code?'selected':''}} {{!old('code') &&auth()->user()->country_code==$country->code?'selected':''}}  value="{{$country->code}}">{{$country->code}}</option>
                @endforeach
              </select>
              <input type="text" class="form-control" value="{{auth()->user()->phone}}" name="phone" placeholder="رقم الجوال" >
        ...
        <div class="col-xs-12 col-sm-6 col-md-6 col-lg-6 padding-right">
                      <div class="form-group form-error">
                        ...
                        <select name="country" id="countries"  class="form-control">
                          @foreach($countries as $country)
                            <option {{auth()->user()->country==$country->id?'selected':''}} value="{{$country->id}}">{{$country->getname()}}</option>
                          @endforeach
                         </select>
                      </div>
        </div>
    ...
    <div class=" col-xs-12 col-sm-12 col-md-12 col-lg-12">
        <p class="cart_navigation clearfix ">
          <a href="#" id="updateprofile" class="button-exclusive btn btn-default" title="Continue shopping">تحديث</a>
        </p>
    </div>
  ...
  <div id="tmcategoryslider" class="block products_block clearfix">
    <ul id="tmcategory-tabs" class="nav nav-tabs clearfix">
      <li class="first_item active">تغيير كلمة المرور</li>
    </ul>
  </div>
 <form action="{{route('changepassword')}}" method="post" id="changepasswordform">
   ...
   <input type="hidden" name="_method" value="PUT">
    <div class="form_content clearfix">
      <div class="form-group form-error">
        <input class="is_required validate account_input form-control" name="oldpassword" type="password" placeholder="كلمة المرور السابقة">
      </div>
      <div class="form-group form-error">
        <input class="is_required validate account_input form-control"  name="password"  type="password" placeholder="كلمة المرور الجديدة">
      </div>
      <div class="form-group form-error">
        <input class="is_required validate account_input form-control" name="password_confirmation"  type="password" placeholder="تأكيد كلمة المرور">
      </div>
      <div class=" col-xs-12 col-sm-12 col-md-12 col-lg-12">
    <p class="cart_navigation clearfix ">
       <a href="#" id="changepassword" class="button-exclusive btn btn-default" title="Continue shopping">تحديث</a>
       </p>
      </div>
    </div>
  </form>
...
$(document).ready(function(){
  ...
  //setting old selections from old()
   ...
   @elseif(isset(auth()->user()->country))
     var cat={{auth()->user()->country}};
     $('#cities').html('');
     $('#cities').append($(`#cities_${cat}`).html());
   @else
