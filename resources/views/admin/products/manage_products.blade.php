      ...
      <div class="box-body">
        <table id="main-table" class="table direction table-bordered table-striped">
          ...
          <tr>
            ...
            <th>{{__('admindash.table.store')}}</th>
            <th>{{__('admindash.product.views')}}</th>
            <th>{{__('admindash.product.points')}}</th>
            ...
            @if(count($products)>0)
              @foreach($products as $product)
                <tr data-id="{{$product->id}}">
                  ...
                  <td>{{$product->quantity?$product->quantity:$product->totalquantity()}}</td>
                  <td>{{$product->store_id==0?setting()->name:$product->store->getname()}}</td>
                  <td>{{$product->views}}</td>
                  ...
                  <td>
                    <a id="editUsers" class="btn btn-default" href="{{route('product.edit',$product->id)}}"><i class="fa fa-edit"></i></a>
                    <form style="display:inline" id="deleteOneForm" action="{{route('product.destroy',$product->id)}}" method="post">
                      @csrf
              ...
              buttons: [
                   ...
                   {    //the select button (confirged in this example to get all ids of selected rows) as deleted button
                       ...
                       action: function ( e, dt, button, config ) {
                            ...
                            $('#deleteModalForm').attr('action',`{{route('product.deletem')}}`);
                            $('#deleteInput').val(ids);
