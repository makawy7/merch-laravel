      ...
      <form role="form" action="{{route('product.store')}}" method="post">
        ...
        <div class="box-body">
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control" id="name_ar" name="title_ar" value="{{old('title_ar')}}">
          </div>
            ...
            <div class="col-md-6">
              ...
              <select class="form-control" name="maincat_id" multiple="" id="maincatselect">
                @foreach($maincats as $maincat)
                  <option {{old('maincat_id')==$maincat->id?'selected':''}} value="{{$maincat->id}}">{{$maincat->getName()}}</option>
                @endforeach
              </select>
            </div>
            <div class="col-md-6">
          ...
          <div class="form-group">
            ...
            <textarea id="head_des" name="head_des" rows="10" cols="80">{{old('head_des')}}</textarea>
          </div>
          <div class="form-group">
            <label for="head_des_ar">{{__('admindash.product.head_des_ar')}}</label>
            <textarea id="head_des_ar" name="head_des_ar" rows="10" cols="80">{{old('head_des_ar')}}</textarea>
          </div>
          <div class="form-group">
          ...
          <div class="form-group col-md-12">
            ...
            <input type="text" id="tags" name="tags" data-role="tagsinput" value="{{old('tags')}}">
          </div>
          <div class="form-group col-md-12">
            <label >{{__('admindash.product.product_priority')}}</label>
            <select class="form-control" id="product_priority" name="product_priority">
              <option value="1" {{old('product_priority')==1?'selected':''}}>1</option>
              <option value="2" {{old('product_priority')==2?'selected':''}}>2</option>
              <option value="3" {{old('product_priority')==3?'selected':''}}>3</option>
              <option value="4" {{old('product_priority')==4?'selected':''}}>4</option>
              <option value="5" {{old('product_priority')==5?'selected':''}}>5</option>
            </select>
          </div>
          <div class="form-group col-md-12">
            <label for="points">{{__('admindash.product.points')}}</label>
            <input type="number" id="points" min="0" step="1" class="form-control" name="reward_points" value="{{old('reward_points')}}">
          </div>
          <div class="form-group col-md-12">
            ...
            <select class="form-control" id="variation" name="variation">
              ...
              <option value="1" {{old('variation')==1?'selected':''}}>{{__('admindash.product.withvariation')}}</option>
            </select>
          </div>
          ...
          <div class="form-group col-md-12 novar">
            ...
            <input type="number" min="1" class="form-control" name="quantity" value="{{old('quantity')}}" id="quantity">
          </div>
        </div>
        <!-- /.box-body -->
        ...
        <div class="box-footer">
          ...
          <a href="{{route('product.index')}}" class="btn btn-default">{{__('admindash.actions.cancel')}}</a>
        </div>
      </form>
      ...
      @if(count($maincats)>0)
          ...
          <span id="subcats_{{$maincat->id}}" class="hide">
              @foreach($maincat->subcats as $subcat)
                <option {{old('subcat_id')==$subcat->id?'selected':''}} value="{{$subcat->id}}">{{$subcat->getname()}}</option>
              @endforeach
          </span>
          <!-- getting all sub categories -->
          @foreach($maincat->subcats as $subcat)
          <span id="types_{{$subcat->id}}" class="hide">
            ...
            <option {{old('type_id')==$type->id?'selected':''}} value="{{$type->id}}">{{$type->getname()}}</option>
            @endforeach
          </span>
          @endforeach
          <!-- getting all brands -->
          @foreach($maincat->subcats as $subcat)
            @foreach($subcat->types as $type)
              <span id="brands_{{$type->id}}" class="hide">
                ...
                <option {{old('brand_id')==$brand->id?'selected':''}} value="{{$brand->id}}">{{$brand->getname()}}</option>
                @endforeach
              </span>
            @endforeach
          @endforeach
...
@if(old('variation')==0)
  ...
  //show price & Quantity fileds
  if($('.novar').hasClass('hide')){
    $('.novar').removeClass('hide');
  }
@else
  $('button[type="submit"]').html(`{{__('admindash.actions.next')}}`);
    //hide price & Quantity fileds
    if(!$('.novar').hasClass('hide')){
      $('.novar').addClass('hide');
    }
@endif
...
$('#variation').on('change',()=>{
  ...
  if($('#variation').find("option:selected").val()==0){
    $('button[type="submit"]').html(`{{__('admindash.actions.submit')}}`);
