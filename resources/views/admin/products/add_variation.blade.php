      ...
      <!-- form start -->
        ...
        <div class="box-body">
          <div class="form-group">
            <button type="button" name="button" class="btn btn-default addnewvariation">اضافة تصنيف جديد</button>
          </div>
          <div class="form-group">
            <button type="button" name="button" data-name="color" class="btn btn-info predefinedvar">اضافة تصنيف اللون</button>
            <button type="button" name="button" data-name="size" class="btn btn-info predefinedvar">اضافة تصنيف المقاس</button>
            <button type="button" name="button" data-name="storage" class="btn btn-info predefinedvar">اضافة تصنيف مساحة التخزين</button>
          </div>
        ...
        <div class="box-footer">
          <a href="{{route('add.options',$product->id)}}" class="btn btn-primary">{{__('admindash.actions.next')}}</a>
        </div>
...
////////////////////////////////////////////Add Predefined Variation///////////////////////////////////////

    $('body').on('click','.predefinedvar',function(){
      var name=$(this).data('name');
      var productid=`{{$product->id}}`;
      var self=this;
      counter++;
      $.ajax({
        url:`{{route('predefinedvar')}}`,
        method:'post',
        data:{name:name,productid:productid,_token:`{{csrf_token()}}`},
        success:function(data){
          $(self).parent().parent().append(`
                    <div class="well" >
                        <div class="form-group">
                          <label for="">{{__('admindash.product.variation_name')}}</label>
                          <input type="text" class="form-control name" value="${data.name}" disabled required name="name">
                          <small class="text-danger name_warning">{{__('admindash.product.variation_name_required')}}</small>
                        </div>
                        <div class="form-group">
                          <label for="">{{__('admindash.product.variation_name_ar')}}</label>
                          <input type="text" class="form-control name_ar" value="${data.name_ar}" disabled required name="name_ar">
                          <small class="text-danger name_ar_warning">{{__('admindash.product.variation_name_ar_required')}}</small>
                        </div>
                        <div class="from-group">
                          <button type="button" name="button" data-id="${data.id}" class="btn btn-danger delete">{{__('admindash.actions.delete')}}</button>
