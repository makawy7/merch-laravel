  ...
  <div class="col-md-12">
    <div class="box">
      <div class="box-header">
        <h3 class="box-title">{{__('admindash.countriesandcities.cities')}}</h3>
      </div>
      <!-- /.box-header -->
      <div class="box-body">
        <table id="main-table" class="table direction table-bordered table-striped">
          ...
          <tr>
            ...
            <th>{{__('admindash.countriesandcities.city')}}</th>
          ...
          <tbody>
            @if(count($cities)>0)
              @foreach($cities as $city)
                <tr data-id="{{$city->id}}">
                  <td><input type="checkbox" id="checkbox_{{$city->id}}" data-id="{{$city->id}}" data-clicked="no" class="checkboxes"></td>
                  <td>{{$city->getname()}}</td>
                  <td>{{$city->country?$city->country->getname():''}}</td>
                  <td>
                    <a id="editUsers" class="btn btn-default" href="{{route('editcity',$city->id)}}"><i class="fa fa-edit"></i></a>
                    <form style="display:inline" id="deleteOneForm" action="{{route('destroycity',$city->id)}}" method="post">
                      @csrf
...
$(document).ready(function() {
    ...
    var table=$('#main-table').DataTable({
      ...
      buttons: {
              ...
              buttons: [
                    { //custom button
                    text: `<sapn style="font-size:14px" class="d-inline">{{trans('admindash.countriesandcities.add_city')}}</sapn>`,
                    className: 'btn btn-default',
                    action: function(){
                      window.location.href='{{route('addcity')}}';  }
                   },
                    { //custom button
                    text: `<sapn style="font-size:14px" class="d-inline">تعديل الترتيب</sapn>`,
                    className: 'btn btn-primary',
                    action: function(){
                      window.location.href='{{route('editcitiesorder')}}';  }
                   },
                   ...
                   {    //the select button (confirged in this example to get all ids of selected rows) as deleted button
                       ...
                       action: function ( e, dt, button, config ) {
                            ...
                            $('#deleteModalForm').attr('action',`{{route('deletemcities')}}`);
                            $('#deleteInput').val(ids);
