  ...
  <div class="col-md-12">
    <div class="box">
      <div class="box-header">
        <h3 class="box-title">{{__('admindash.shippingmethods.shippingmethods')}}</h3>
      </div>
      <!-- /.box-header -->
      <div class="box-body">
        <table id="main-table" class="table direction table-bordered table-striped">
          ...
          <tr>
            ...
            <th>{{__('admindash.shippingmethods.shippingmethod')}}</th>
            <th>{{__('admindash.shippingmethods.cost')}}</th>
            <th>{{__('admindash.shippingmethods.deliverytime')}}</th>
          ...
          <tbody>
            @if(count($shippingmethods)>0)
              @foreach($shippingmethods as $shippingmethod)
                <tr data-id="{{$shippingmethod->id}}">
                  <td><input type="checkbox" id="checkbox_{{$shippingmethod->id}}" data-id="{{$shippingmethod->id}}" data-clicked="no" class="checkboxes"></td>
                  <td>{{$shippingmethod->getname()}}</td>
                  <td>{{$shippingmethod->cost}} {{__('admindash.constants.usd')}}</td>
                  <td>{{$shippingmethod->deliverytime}} {{__('admindash.constants.days')}}</td>
                  <td>
                    <a id="editUsers" class="btn btn-default" href="{{route('editshippingmethod',$shippingmethod->id)}}"><i class="fa fa-edit"></i></a>
                    <form style="display:inline" id="deleteOneForm" action="{{route('destroyshippingmethod',$shippingmethod->id)}}" method="post">
                      @csrf
...
$(document).ready(function() {
    ...
    var table=$('#main-table').DataTable({
      ...
      buttons: {
              ...
              buttons: [
                    { //custom button
                    text: `<sapn style="font-size:14px" class="d-inline">{{trans('admindash.shippingmethods.add_shippingmethod')}}</sapn>`,
                    className: 'btn btn-default',
                    action: function(){
                      window.location.href='{{route('addshippingmethod')}}';  }
                   },
                   ...
                   {    //the select button (confirged in this example to get all ids of selected rows) as deleted button
                       ...
                       action: function ( e, dt, button, config ) {
                            ...
                            $('#deleteModalForm').attr('action',`{{route('deletemshippingmethods')}}`);
                            $('#deleteInput').val(ids);
