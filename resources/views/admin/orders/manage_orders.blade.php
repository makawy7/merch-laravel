  ...
  <div class="col-md-12">
    <div class="box">
      <div class="box-header">
        <h3 class="box-title">{{__('admindash.orders.orders')}}</h3>
      </div>
      <!-- /.box-header -->
      <div class="box-body">
        <table id="main-table" class="table direction table-bordered table-striped">
          ...
          <tr>
            ...
            <th>{{__('admindash.orders.status')}}</th>
            ...
            @if(count($orders)>0)
              @foreach($orders as $order)
                <tr data-id="{{$order->id}}">
                  ...
                  <td>{{$order->status==0?trans('admindash.orders.awaitingconfirm'):$order->getstatus->getname()}}</td>
                  <td>
