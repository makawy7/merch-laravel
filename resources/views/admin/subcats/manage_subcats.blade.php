  ...
  <div class="col-md-12">
    <div class="box">
      <div class="box-header">
        <h3 class="box-title">{{__('admindash.subcat.subcats')}}</h3>
      </div>
      <!-- /.box-header -->
      <div class="box-body">
            ...
            @if(count($subcats)>0)
              @foreach($subcats as $subcat)
                <tr data-id="{{$subcat->id}}">
                  <td><input type="checkbox" id="checkbox_{{$subcat->id}}" data-id="{{$subcat->id}}" data-clicked="no" class="checkboxes"></td>
                  <td>{{$subcat->getname()}}</td>
                  <td>{{$subcat->maincat->getname()}}</td>
                  <td>
                    <a id="editUsers" class="btn btn-default" href="{{route('subcat.edit',$subcat->id)}}"><i class="fa fa-edit"></i></a>
                    <form style="display:inline" id="deleteOneForm" action="{{route('subcat.destroy',$subcat->id)}}" method="post">
                      @csrf
...
$(document).ready(function() {
    ...
    var table=$('#main-table').DataTable({
      ...
      buttons: {
              ...
              buttons: [
                    { //custom button
                    text: `<sapn style="font-size:14px" class="d-inline">{{trans('admindash.subcat.addsubcat')}}</sapn>`,
                    className: 'btn btn-default',
                    action: function(){
                      window.location.href='{{route('subcat.create')}}';  }
                   },
                   ...
                   {    //the select button (confirged in this example to get all ids of selected rows) as deleted button
                       ...
                       action: function ( e, dt, button, config ) {
                            ...
                            $('#deleteModalForm').attr('action',`{{route('subcat.deletem')}}`);
                            $('#deleteInput').val(ids);
