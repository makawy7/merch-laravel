  ...
  <div class="col-md-12">
    <div class="box">
      <div class="box-header">
        <h3 class="box-title">{{__('admindash.type.types')}}</h3>
      </div>
      <!-- /.box-header -->
      <div class="box-body">
          ...
          <tbody>
            @if(count($types)>0)
              @foreach($types as $type)
                <tr data-id="{{$type->id}}">
                  <td><input type="checkbox" id="checkbox_{{$type->id}}" data-id="{{$type->id}}" data-clicked="no" class="checkboxes"></td>
                  <td>{{$type->getname()}}</td>
                  <td>{{$type->subcat->getname()}}</td>
                  <td>{{$type->subcat->maincat->getname()}}</td>
                  <td>
                    <a id="editUsers" class="btn btn-default" href="{{route('type.edit',$type->id)}}"><i class="fa fa-edit"></i></a>
                    <form style="display:inline" id="deleteOneForm" action="{{route('type.destroy',$type->id)}}" method="post">
                      @csrf
...
$(document).ready(function() {
    ...
    var table=$('#main-table').DataTable({
      ...
      buttons: {
              ...
              buttons: [
                    { //custom button
                    text: `<sapn style="font-size:14px" class="d-inline">{{trans('admindash.type.addtype')}}</sapn>`,
                    className: 'btn btn-default',
                    action: function(){
                      window.location.href='{{route('type.create')}}';  }
                   },
                   ...
                   {    //the select button (confirged in this example to get all ids of selected rows) as deleted button
                       ...
                       action: function ( e, dt, button, config ) {
                            ...
                            $('#deleteModalForm').attr('action',`{{route('type.deletem')}}`);
                            $('#deleteInput').val(ids);
