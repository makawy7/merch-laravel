  ...
  <div class="col-md-12">
    <div class="box">
      <div class="box-header">
        <h3 class="box-title">{{__('admindash.paymentmethods.paymentmethods')}}</h3>
      </div>
      <!-- /.box-header -->
      <div class="box-body">
        <table id="main-table" class="table direction table-bordered table-striped">
          ...
          <tr>
            ...
            <th>{{__('admindash.paymentmethods.paymentmethod')}}</th>
            ...
            @if(count($paymentmethods)>0)
              @foreach($paymentmethods as $paymentmethod)
                <tr data-id="{{$paymentmethod->id}}">
                  <td><input type="checkbox" id="checkbox_{{$paymentmethod->id}}" data-id="{{$paymentmethod->id}}" data-clicked="no" class="checkboxes"></td>
                  <td>{{$paymentmethod->getname()}}</td>
                  <td>
                    <a id="editUsers" class="btn btn-default" href="{{route('editpaymentmethod',$paymentmethod->id)}}"><i class="fa fa-edit"></i></a>
                    <form style="display:inline" id="deleteOneForm" action="{{route('destroypaymentmethod',$paymentmethod->id)}}" method="post">
                      @csrf
...
$(document).ready(function() {
    ...
    var table=$('#main-table').DataTable({
      ...
      buttons: {
              ...
              buttons: [
                    { //custom button
                    text: `<sapn style="font-size:14px" class="d-inline">{{trans('admindash.paymentmethods.addpaymentmethod')}}</sapn>`,
                    className: 'btn btn-default',
                    action: function(){
                      window.location.href='{{route('addpaymentmethod')}}';  }
                   },
                    { //custom button
                    text: `<sapn style="font-size:14px" class="d-inline">تعديل الترتيب</sapn>`,
                    className: 'btn btn-primary',
                    action: function(){
                      window.location.href='{{route('editpaymentmethodsorder')}}';  }
                   },
                   ...
                   {    //the select button (confirged in this example to get all ids of selected rows) as deleted button
                       ...
                       action: function ( e, dt, button, config ) {
                            ...
                            $('#deleteModalForm').attr('action',`{{route('deletempaymentmethods')}}`);
                            $('#deleteInput').val(ids);
