      ...
      <div class="box-body">
        <table id="main-table" class="table direction table-bordered table-striped">
          ...
          <tr>
            ...
            <th>{{__('admindash.account.name')}}</th>
            ...
            @if(count($users)>0)
              @foreach($users as $user)
                <tr data-id="{{$user->id}}">
                  ...
                  <td>
                    <a id="editUsers" class="btn btn-success" href="{{route('user.setadmin',$user->id)}}">{{__('admindash.account.setasadmin')}}</a>
                    <a id="editUsers" class="btn btn-default" href="{{route('user.edit',$user->id)}}"><i class="fa fa-edit"></i></a>
                    <form style="display:inline" id="deleteOneForm" action="{{route('user.destroy',$user->id)}}" method="post">
                      @csrf
              ...
              buttons: [
                   ...
                   {    //the select button (confirged in this example to get all ids of selected rows) as deleted button
                       ...
                       action: function ( e, dt, button, config ) {
                            ...
                            $('#deleteModalForm').attr('action',`{{route('user.deletem')}}`);
                            $('#deleteInput').val(ids);
