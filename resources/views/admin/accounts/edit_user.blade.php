  ...
  <div class="col-md-12">
    ...
    <div class="box box-primary">
      <div class="box-header with-border">
        <h3 class="box-title">{{__('admindash.account.edituser')}}</h3>
      </div>
      <!-- /.box-header -->
      <!-- form start -->
        ...
        <div class="box-body">
          ...
          <div class="form-group">
                          ...
                          <div class="input-group date">
                            ...
                            <input type="text" class="form-control pull-right" value="{{old('birthday')?old('birthday'):$user->birthday}}" name="birthday" id="datepicker">
                          </div>
                          <!-- /.input group -->
          </div>
          <div class="form-group">
            ...
            <select class="form-control" name="gender">
              <option {{old('gender')=='male'?'selected':''}} {{!old('gender') && $user->gender=='male'?'selected':''}} value="male">{{__('admindash.constants.male')}}</option>
              <option {{old('gender')=='female'?'selected':''}} {{!old('gender') && $user->gender=='female'?'selected':''}} value="female">{{__('admindash.constants.female')}}</option>
            </select>
          </div>
          <div class="form-group">
            ...
            <select class="" name="country_code">
              @foreach($countries as $country)
                <option {{$user->country_code==$country->code?'selected':''}} value="{{$country->code}}">{{$country->code}}</option>
              @endforeach
            </select>
            <input type="text" id="phonenumber" class="form-control" value="{{$user->phone}}" name="phone" >
          </div>
        </div>
        <!-- /.box-body -->
