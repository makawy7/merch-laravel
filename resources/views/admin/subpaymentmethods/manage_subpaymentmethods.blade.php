  ...
  <div class="col-md-12">
    <div class="box">
      <div class="box-header">
        <h3 class="box-title">{{__('admindash.subpayments.subpayments')}}</h3>
      </div>
      <!-- /.box-header -->
      <div class="box-body">
        <table id="main-table" class="table direction table-bordered table-striped">
          ...
          <tr>
            ...
            <th>{{__('admindash.subpayments.subpayment')}}</th>
            ...
            @if(count($paymentmethods)>0)
              @foreach($paymentmethods as $paymentmethod)
                <tr data-id="{{$paymentmethod->id}}">
                  ...
                  <td>
                    <a id="editUsers" class="btn btn-default" href="{{route('subpaymentmethods.edit',$paymentmethod->id)}}"><i class="fa fa-edit"></i></a>
                    <form style="display:inline" id="deleteOneForm" action="{{route('subpaymentmethods.destroy',$paymentmethod->id)}}" method="post">
                      @csrf
    ...
    var table=$('#main-table').DataTable({
      ...
      buttons: {
              ...
              buttons: [
                    ...
                    action: function(){
                      window.location.href='{{route('subpaymentmethods.create')}}';  }
                   },
                   ...
                   {    //the select button (confirged in this example to get all ids of selected rows) as deleted button
                       ...
                       action: function ( e, dt, button, config ) {
                            ...
                            $('#deleteModalForm').attr('action',`{{route('subpaymentmethods.deletem')}}`);
                            $('#deleteInput').val(ids);
