  ...
  <div class="col-md-12">
    <div class="box">
      <div class="box-header">
        <h3 class="box-title">{{__('admindash.mainpage.banners')}}</h3>
      </div>
      <!-- /.box-header -->
      <div class="box-body">
        <table id="main-table" class="table direction table-bordered table-striped">
          ...
          <tr>
            ...
            <th>{{__('admindash.mainpage.title')}}</th>
            ...
            @if(count($banners)>0)
              @foreach($banners as $banner)
                <tr data-id="{{$banner->id}}">
                  <td><input type="checkbox" id="checkbox_{{$banner->id}}" data-id="{{$banner->id}}" data-clicked="no" class="checkboxes"></td>
                  <td>{{$banner->gettitle()}}</td>
                  <td> <a href="{{$banner->url}}"> <img style="height:100px" src="{{url('storage/images/banners/'.$banner->image)}}" alt=""> </a></td>
                  <td>
                    <a id="editUsers" class="btn btn-default" href="{{route('editbanner',$banner->id)}}"><i class="fa fa-edit"></i></a>
                    <form style="display:inline" id="deleteOneForm" action="{{route('destroybanner',$banner->id)}}" method="post">
                      @csrf
...
$(document).ready(function() {
    ...
    var table=$('#main-table').DataTable({
      ...
      buttons: {
              ...
              buttons: [
                    { //custom button
                    text: `<sapn style="font-size:14px" class="d-inline">{{trans('admindash.mainpage.addbanner')}}</sapn>`,
                    className: 'btn btn-default',
                    action: function(){
                      window.location.href='{{route('createbanner')}}';  }
                   },
                   ...
                   {    //the select button (confirged in this example to get all ids of selected rows) as deleted button
                       ...
                       action: function ( e, dt, button, config ) {
                            ...
                            $('#deleteModalForm').attr('action',`{{route('deletembanners')}}`);
                            $('#deleteInput').val(ids);
