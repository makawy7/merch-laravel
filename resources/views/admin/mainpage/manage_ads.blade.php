  ...
  <div class="col-md-12">
    <div class="box">
      <div class="box-header">
        <h3 class="box-title">{{__('admindash.mainpage.bannerads')}}</h3>
      </div>
      <!-- /.box-header -->
      <div class="box-body">
        <table id="main-table" class="table direction table-bordered table-striped">
          ...
          <tbody>
            @if(count($ads)>0)
              @foreach($ads as $ad)
                <tr data-id="{{$ad->id}}">
                  <td><input type="checkbox" id="checkbox_{{$ad->id}}" data-id="{{$ad->id}}" data-clicked="no" class="checkboxes"></td>
                  <td>{{$ad->gettitle()}}</td>
                  <td> <a href="{{$ad->url}}"> <img style="height:100px" src="{{url('storage/images/ads/'.$ad->image)}}" alt=""> </a></td>
                  <td>
                    <a id="editUsers" class="btn btn-default" href="{{route('editad',$ad->id)}}"><i class="fa fa-edit"></i></a>
                    <form style="display:inline" id="deleteOneForm" action="{{route('destroyad',$ad->id)}}" method="post">
                      @csrf
...
$(document).ready(function() {
    ...
    var table=$('#main-table').DataTable({
      ...
      buttons: {
              ...
              buttons: [
                    { //custom button
                    text: `<sapn style="font-size:14px" class="d-inline">{{trans('admindash.mainpage.addbannerad')}}</sapn>`,
                    className: 'btn btn-default',
                    action: function(){
                      window.location.href='{{route('createad')}}';  }
                   },
                   ...
                   {    //the select button (confirged in this example to get all ids of selected rows) as deleted button
                       ...
                       action: function ( e, dt, button, config ) {
                            ...
                            $('#deleteModalForm').attr('action',`{{route('deletemads')}}`);
                            $('#deleteInput').val(ids);
